...
func NewMailSender(conf *config.MailSender) *MailManager {
	return &MailManager{
		host:     conf.Host,
		port:     conf.Port,
...
func (ms *MailManager) SendLetter(ctx context.Context, to string, subject string, htmlBody string) error {
