// Code generated by mockery v2.53.6. DO NOT EDIT.

package mockCardRep

import (
	context "context"

	dto "github.com/go-park-mail-ru/2026_1_Clac_Clac/monolith/internal/card/repository/dto"
	mock "github.com/stretchr/testify/mock"

	uuid "github.com/google/uuid"
)

// CardRep is an autogenerated mock type for the CardRep type
type CardRep struct {
	mock.Mock
}

// CreateCard provides a mock function with given fields: ctx, newCard
func (_m *CardRep) CreateCard(ctx context.Context, newCard dto.NewCard) (int, error) {
	ret := _m.Called(ctx, newCard)

	if len(ret) == 0 {
		panic("no return value specified for CreateCard")
	}

	var r0 int
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, dto.NewCard) (int, error)); ok {
		return rf(ctx, newCard)
	}
	if rf, ok := ret.Get(0).(func(context.Context, dto.NewCard) int); ok {
		r0 = rf(ctx, newCard)
	} else {
		r0 = ret.Get(0).(int)
	}

	if rf, ok := ret.Get(1).(func(context.Context, dto.NewCard) error); ok {
		r1 = rf(ctx, newCard)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// DeleteCard provides a mock function with given fields: ctx, linkCard
func (_m *CardRep) DeleteCard(ctx context.Context, linkCard uuid.UUID) error {
	ret := _m.Called(ctx, linkCard)

	if len(ret) == 0 {
		panic("no return value specified for DeleteCard")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID) error); ok {
		r0 = rf(ctx, linkCard)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// GetCard provides a mock function with given fields: ctx, linkCard
func (_m *CardRep) GetCard(ctx context.Context, linkCard uuid.UUID) (dto.InfoCard, error) {
	ret := _m.Called(ctx, linkCard)

	if len(ret) == 0 {
		panic("no return value specified for GetCard")
	}

	var r0 dto.InfoCard
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID) (dto.InfoCard, error)); ok {
		return rf(ctx, linkCard)
	}
	if rf, ok := ret.Get(0).(func(context.Context, uuid.UUID) dto.InfoCard); ok {
		r0 = rf(ctx, linkCard)
	} else {
		r0 = ret.Get(0).(dto.InfoCard)
	}

	if rf, ok := ret.Get(1).(func(context.Context, uuid.UUID) error); ok {
		r1 = rf(ctx, linkCard)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// ReorderCard provides a mock function with given fields: ctx, updatingPlaceCard
func (_m *CardRep) ReorderCard(ctx context.Context, updatingPlaceCard dto.PlaceCard) error {
	ret := _m.Called(ctx, updatingPlaceCard)

	if len(ret) == 0 {
		panic("no return value specified for ReorderCard")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, dto.PlaceCard) error); ok {
		r0 = rf(ctx, updatingPlaceCard)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// UpdateCardDetails provides a mock function with given fields: ctx, updatedCard
func (_m *CardRep) UpdateCardDetails(ctx context.Context, updatedCard dto.UpdatingCardDetails) error {
	ret := _m.Called(ctx, updatedCard)

	if len(ret) == 0 {
		panic("no return value specified for UpdateCardDetails")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, dto.UpdatingCardDetails) error); ok {
		r0 = rf(ctx, updatedCard)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// NewCardRep creates a new instance of CardRep. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewCardRep(t interface {
	mock.TestingT
	Cleanup(func())
}) *CardRep {
	mock := &CardRep{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
