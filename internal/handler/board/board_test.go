...
func TestGetUserBoards(t *testing.T) {
	...
	tests := []struct {
		...
		{
			nameTest:           "Error user not authorized (nil context)",
			ctxValue:           nil,
			mockBehavior:       nil,
		...
		{
			nameTest:           "Error context value is not UUID (wrong type)",
		...
		},
		{
			...
			ctxValue: targetUserID,
			mockBehavior: func(m *mockBoardService.BoardService) {
				m.On("GetBoards", mock.Anything, targetUserID).Return([]models.Board{})
			},
			expectedStatusCode: http.StatusOK,
			expectedResponse:   "[]\n",
		},
